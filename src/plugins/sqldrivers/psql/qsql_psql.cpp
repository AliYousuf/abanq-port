...
static QSqlError qMakeError( const QString & err, int type, const QPSQLPrivate * p ) {
  QString msg = QString( PQerrorMessage( p->connection ) );
...
QVariant QPSQLResult::data( int i ) {
  ...
  const QString val = ( d->isUtf8 && ptype != BYTEAOID ) ?
                      QString::fromUtf8( PQgetvalue( d->result, at() % LIMIT_RESULT, i ) ) :
...
QPSQLDriver::QPSQLDriver( QObject * parent, const char *name ) : FLSqlDriver( parent, name ? name : "QPSQL" ), pro( QPSQLDriver::Version6 ) {
  init();
}

QPSQLDriver::QPSQLDriver( PGconn * conn, QObject * parent, const char *name ) : FLSqlDriver( parent, name ? name : "QPSQL" ), pro( QPSQLDriver::Version6 ) {
  init();
  d->connection = conn;
  if ( conn ) {
...
bool QPSQLDriver::open( const QString & db, const QString & user, const QString & password, const QString & host, int port, const QString & connOpts ) {
  if ( isOpen() )
    close();
  ...
  switch ( t ) {
    case QVariant::Bool: {
      ...
      if ( s == QApplication::tr( "Sí" ).left( 1 ).upper() )
        ...
        if ( s == QApplication::tr( "No" ).left( 1 ).upper() )
          res = "='f'";
    }
    break;
    case QVariant::Date:
...
#ifndef FL_QUICK_CLIENT
  if ( cur && cur->metadata() && cur->sort().count() > 1 ) {
    d->checkLock = false;
      ...
      case QVariant::ByteArray: {
        ...
        for ( int i = 0; i < ( int ) ba.size(); ++i ) {
          ...
          if ( uc > 40 && uc < 92 ) {
            ...
            r += "\\\\";
            r += QString::number( ( unsigned char ) ba[ i ],
                                  8 ).rightJustify( 3, '0', true );
          }
        }
        r += "'";
        break;
...
void QPSQLDriver::Mr_Proper() {
  QString mproperMsg( tr( "Este proceso puede tener una larga duración, dependiendo\n"
                          ...
                          "antes de empezar.\n\n¿ Quiere continuar ?" ) );
  int res = QMessageBox::question( 0, tr( "Mr. Proper" ), mpropeMsg, QMessageBox::Yes, QMessageBox::No );
  if ( res == QMessageBox::Yes ) {
    d->activeCreateIndex = false;
    db_->dbAux()->transaction();
    QSqlQuery qry( QString::null, db_->dbAux() );
  ...
  while ( qry.next() ) {
    ...
    if ( !item.isEmpty() && item != "0" ) {
      if ( skipPk ) {
        ...
        if ( primaryKeyValue == qry.value( 0 ).toString() ) {
          item += "@##" + pk + "=" + qry.value( 0 ).toString();
          rows.append( item );
        }
    }
