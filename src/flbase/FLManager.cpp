...
void FLManager::loadTables() {
  ...
  if ( !db_->dbAux() )
    return ;
  if ( !listTables )
    listTables = new QStringList();
  else
    listTables->clear();

  *listTables = db_->dbAux() ->tables();
#endif
}
...
void FLManager::init() {
  ...
  if ( !q.exec( "SELECT * FROM flsettings WHERE flkey = 'sysmodver'" ) ) {
    q.exec( "DROP TABLE flsettings" );
    tmpTMD = createSystemTable( "flsettings" );
  }

  if ( !dictKeyMetaData ) {
    dictKeyMetaData = new QDict < QString > ( 277 );
    dictKeyMetaData->setAutoDelete( true );
  } else
    dictKeyMetaData->clear();

  q.exec( "SELECT tabla,xml FROM flmetadata" );
  while ( q.next() )
    dictKeyMetaData->insert( q.value( 0 ).toString(), new QString( q.value( 1 ).toString() ) );

  q.exec( "SELECT * FROM flsettings WHERE flkey = 'sysmodver'" );
  if ( !q.next() ) {
    q.exec( "DROP TABLE flmetadata" );
    tmpTMD = createSystemTable( "flmetadata" );

    QSqlCursor c( "flmetadata", true, db_->dbAux() );
    QSqlRecord * buffer;
    QDictIterator < QString > it( *dictKeyMetaData );
  ...
  if ( !cacheMetaData ) {
    cacheMetaData = new QCache < FLTableMetaData >( 50, 101 );
    cacheMetaData->setAutoDelete( true );
  }

  if ( !cacheAction ) {
    cacheAction = new QCache < FLAction >( 50, 101 );
    cacheAction->setAutoDelete( true );
  }

  if ( !cacheMetaDataSys ) {
    cacheMetaDataSys = new QCache < FLTableMetaData >( 50, 101 );
    cacheMetaDataSys->setAutoDelete( true );
  }

  if ( !cacheLargeValues ) {
    cacheLargeValues = new QCache < QVariant >( 50, 101 );
    cacheLargeValues->setAutoDelete( true );
  }
}

bool FLManager::existsTable( const QString & n ) const {
#ifndef FL_QUICK_CLIENT
  if ( !db_->dbAux() || n.isEmpty() )
    return false;
  if ( listTables )
    return ( listTables->contains( n ) ? true : ( db_->dbAux() ->tables().contains( n ) ) );
  else
...
bool FLManager::checkMetaData( FLTableMetaData * tmd1, FLTableMetaData * tmd2 ) {
  ...
  while (( field1 = it.current() ) != 0 ) {
    ...
    if ( field1->isUnique() != field2->isUnique() || field1->isIndex() != field2->isIndex() )
      return false;
    if ( field1->length() != field2->length() || field1->partDecimal() != field2->partDecimal() || field1->partInteger() != field2->partInteger() )
      return false;
  }
...
#ifndef FL_QUICK_CLIENT
  QString mtd( db_->managerModules()->contentCached( n + ".mtd" ) );
...
FLFieldMetaData *FLManager::metadataField( QDomElement * field, bool v, bool ed ) {
  ...
  FLFieldMetaData *f = new FLFieldMetaData( n, QT_TRANSLATE_NOOP( "MetaData", a ), aN, iPK, t,
    ...
    for ( QStringList::Iterator it = fL.begin(); it != fL.end(); ++it ) {
      ...
      if ( mtdAux ) {
        ...
        if ( fmtdAux ) {
          ...
          tmd->addFieldMD( new FLFieldMetaData( field, fmtdAux->alias(), true, false, typeAux, fmtdAux->length(), false, fmtdAux->visible(), fmtdAux->editable(), fmtdAux->partInteger(), fmtdAux->partDecimal(), false, false, false, QVariant(), false, QString::null, fmtdAux->visibleGrid(), true, false ) );
        }
      }
    }
  }
...
void FLManager::cleanupMetaData() {
  ...
  q.exec( "SELECT tabla,xml FROM flmetadata" );
  while ( q.next() )
    dictKeyMetaData->replace( q.value( 0 ).toString(), new QString( q.value( 1 ).toString() ) );
  ...
  while ( q.next() ) {
    ...
    if ( !tmd )
      qWarning( "FLManager::cleanupMetaData " + QApplication::tr( "No se ha podido crear los metadatos para la tabla %1" ).arg( table ) );
    ...
    dictKeyMetaData->replace( table, new QString( q.value( 1 ).toString() ) );
  }
#endif
}
...
FLTableMetaData *FLManager::metadata( const QString & n, bool quick ) {
  ...
  QString key;
  QString *dictKey = 0;
  QString stream = db_->managerModules()->contentCached( n + ".mtd", &key );
  bool newTable = false;
...
#ifndef FL_QUICK_CLIENT
  if ( !quick ) {
    if ( db_->canRegenTables() && !isSystemTable( n ) ) {
      QSqlRecord * buffer;
      QString key2;
      dictKey = dictKeyMetaData->find( n );
      if ( dictKey )
        key2 = *dictKey;
      if ( key2.isEmpty() ) {
        QSqlCursor c( "flmetadata", true, db_->dbAux() );
        ...
        if ( key.isEmpty() ) {
          ...
          dictKeyMetaData->replace( n, dictKey );
        } else {
  ...
  if ( dictKey ) {
    if ( cacheMetaData && !isSystemTable( n ) )
      ret = cacheMetaData->find( *dictKey );
    else
      if ( cacheMetaDataSys )
        ret = cacheMetaDataSys->find( *dictKey );
    if ( ret ) {
      FLAccessControlLists * acl = static_cast<FLApplication *>( qApp ) ->acl();
      if ( acl )
        acl->process( ret );
  ...
  if ( dictKey && !ret->isQuery() ) {
    if ( cacheMetaData && !isSystemTable( n ) )
      cacheMetaData->insert( *dictKey, ret );
    else
      if ( cacheMetaDataSys )
        cacheMetaDataSys->insert( *dictKey, ret );
...
FLParameterQuery *FLManager::queryParameter( QDomElement * parameter ) {
  ...
  return new FLParameterQuery( name, QT_TRANSLATE_NOOP( "Queries", alias ), t );
}
...
FLSqlQuery *FLManager::query( const QString & n, QObject * parent ) {
  ...
  if ( !FLUtil::domDocumentSetContent( doc, db_->managerModules()->contentCached( n + ".qry" ) ) ) {
#ifdef FL_DEBUG
...
FLAction *FLManager::action( const QString & n ) {
  ...
  for ( QStringList::Iterator it = listModules.begin(); it != listModules.end(); ++it ) {
    contentActions = db_->managerModules()->contentCached(( *it ) + ".xml" );
    if ( contentActions.find( "<name>" + n + "</name>", 0, true ) != -1 )
      break;
  }
...
QString FLManager::formatAssignValueLike( const QString & fieldName, int t, const QVariant & v, const bool upper ) {
  ...
  QString fName(( upper && isText ? "upper(" + fieldName + ")" : fieldName ) );
  return fName + " " + formatV;
}

QString FLManager::formatAssignValueLike( const QString & fieldName, FLFieldMetaData * fMD, const QVariant & v, const bool upper ) {
  if ( !fMD )
    return "1 = 1";
...
QString FLManager::formatAssignValue( const QString & fieldName, FLFieldMetaData * fMD, const QVariant & v, const bool upper ) {
  if ( !fMD )
    return "1 = 1";
...
QVariant FLManager::fetchLargeValue( const QString & refKey ) const {
  ...
  QVariant * cachedV = cacheLargeValues->find( sha );
  if ( cachedV )
  ...
  if ( qryLarge.exec( "SELECT contenido FROM " + tableLarge + " WHERE refkey='" + refKey + "'" ) &&
    ...
    if ( v.isValid() )
      cacheLargeValues->insert( sha, new QVariant( v ) );
    return v;
  }
  return QVariant();
}
