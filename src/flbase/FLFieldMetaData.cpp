...
FLFieldMetaData::FLFieldMetaData( const QString & n, const QString & a, bool aN, bool iPK,
  ...
  d = new FLFieldMetaDataPrivate( n, a, aN, iPK, t, l, c, v, ed, pI, pD, iNX,
                                  uNI, coun, defValue, oT, rX, vG, gen, iCK );
}
  ...
  for ( relation = rl->first(); relation; relation = rl->next() ) {
    ...
    if ( relation->cardinality() == FLRelationMetaData::RELATION_M1 ) {
      d->relationM1_ = new FLRelationMetaData( relation->foreignTable(), relation->foreignField(),
          relation->cardinality(), relation->deleteCascade(), relation->updateCascade(), relation->checkIn() );
