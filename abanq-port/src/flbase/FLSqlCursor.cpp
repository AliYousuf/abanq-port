...
FLSqlCursor::~FLSqlCursor() {
  if ( !d->transactionsOpened.isEmpty() )
    rollbackOpened( -1, tr( "Se han detectado transacciones no finalizadas en la última operación.\n" ) +
  ...
  if ( d->modeAccess_ == INSERT || d->modeAccess_ == EDIT ) {
    ...
    while ( ( field = it.current() ) != 0 ) {
      ...
      if ( fMD && s.isValid() && !s.isNull() ) {
        if ( !field->relationM1() ) {
          msg += "\n" + tr( "FLSqlCursor : Error en metadatos, el campo %1 tiene un campo asociado pero no existe relación muchos a uno" ).arg( d->metadata_->name() + ":" + fiName );
          continue;
        }
        ...
        if ( !d->buffer_->isNull( pK ) && s.isValid() && !s.isNull() ) {
          ...
          if ( q.next() )
            msg += "\n" + d->metadata_->name() + ":" + field->alias() +
                   tr( " : Requiere valores únicos, y ya hay otro registro con el valor %1 en este campo" ).arg( s.toString() );
        }
      }
      ...
      if ( field->isPrimaryKey() && d->modeAccess_ == INSERT && s.isValid() && !s.isNull() ) {
        ...
        q.exec();
        if ( q.next() )
          msg += "\n" + d->metadata_->name() + ":" + field->alias() + tr( " : Es clave primaria y requiere valores únicos, y ya hay otro registro con el valor %1 en este campo" ).arg( s.toString() );
      }
      ...
      if ( fieldListCK && !checkedCK && d->modeAccess_ == INSERT ) {
        if ( !fieldListCK->isEmpty() ) {
          ...
          q.exec();
          if ( q.next() )
            msg += "\n" + fields + tr( " : Requiere valor único, y ya hay otro registro con el valor %1" ).arg( valuesFields );
          checkedCK = true;
        }
      }
    }
...
void FLSqlCursor::openFormInMode( int m, bool cont ) {
  ...
  if ( d->action_->formRecord().isEmpty() ) {
    QMessageBox::warning( qApp->focusWidget(), tr( "Aviso" ),
                          tr( "No hay definido ningún formulario para manejar registros de esta tabla" ), QMessageBox::Ok, 0, 0 );
    return ;
  } else {
...
bool FLSqlCursor::transaction( bool lock ) {
  ...
  if ( transaction_ == 0 && d->db_->canTransaction() ) {
    static_cast<FLApplication *>( qApp ) ->statusHelpMsg( tr( "Iniciando transacción..." ) );
...
bool FLSqlCursor::rollback() {
  ...
  if ( transaction_ == 0 && d->db_->canTransaction() ) {
    static_cast<FLApplication *>( qApp ) ->statusHelpMsg( tr( "Deshaciendo transacción..." ) );
...
bool FLSqlCursor::commit( bool notify ) {
  ...
  if ( transaction_ == 0 && d->db_->canTransaction() ) {
    static_cast<FLApplication *>( qApp ) ->statusHelpMsg( tr( "Terminando transacción..." ) );
    if ( d->db_->db()->commit() ) {
      if ( !d->db_->canSavePoint() ) {
        if ( currentSavePoint ) {
          delete currentSavePoint;
