...
FLTableDB::FLTableDB( QWidget * parent, const char *name ) : FLWidgetTableDB( parent, name ),
    ...
    findHidden_( false ), filterHidden_( false ), fakeEditor_( 0 ) {
  ...
  if ( !topWidget ) {
    ...
    cursor_ = static_cast<FLFormDB *>( topWidget ) ->cursor();
    setFont( qApp->font() );
  }
  ...
  if ( checkColumnEnabled_ ) {
    if ( !checkColumnVisible_ ) {
      ...
      if ( !fieldCheck ) {
        ...
        fieldCheck = new FLFieldMetaData( fieldNameCheckColumn_, aliasCheckColumn_, true, false, FLFieldMetaData::Check, 0, false, true,
...
#ifdef FL_TEST
  static_cast<FLApplication *>( qApp ) ->continueTesting( FLTester::RECORD_DELETED, "VOID" );
#endif
}
...
#ifdef FL_TEST
  QString formName = "formRecord" + cursor_->action() ->name();
...
void FLTableDB::initCursor() {
  ...
  if ( !tableName_.isEmpty() ) {
    ...
    if ( foreignField_.isEmpty() || fieldRelation_.isEmpty() ) {
      ...
      FLSqlCursor * cursorTopWidget = static_cast<FLFormDB *>( topWidget ) ->cursor();
  ...
  if ( cursorAux && topWidget->isA( "FLFormSearchDB" ) ) {
    ...
    static_cast<FLFormSearchDB *>( topWidget ) ->setCursor( cursor_ );
  }
}
...
void FLTableDB::setOrderCols( QStringList & fields ) {
  ...
  for ( j = sortColumn_; j < i; ++j )
    if ( fields[j] != tMD->fieldAliasToName( horizHeader->label( j ) ) )
      break;
  if ( j == i )
    return;
...
void FLTableDB::seekCursor() {
  ...
  if ( !textSearch.contains( "'" ) && !textSearch.contains( "\\" ) ) {
    QString where( ( cursor_->curFilter().isEmpty() ? " where " : " where " + cursor_->curFilter() + " and " ) );
    where += cursor_->db()->manager()->formatAssignValueLike( fN, sortField_, textSearch, true );
    QString sql( "select " + fN + " from " + tMD->name() + where + " order by " + fN + QString( orderAsc_ ? " ASC" : " DESC" ) + " limit 1" );
      ...
      for ( int j = 2; j < 5; ++j ) {
        ...
        switch ( type ) {
          ...
          case QVariant::Date: {
            ...
            static_cast<QDateEdit *>( editor_ )->setOrder( QDateEdit::DMY );
            static_cast<QDateEdit *>( editor_ )->setAutoAdvance( true );
            static_cast<QDateEdit *>( editor_ )->setSeparator( "-" );
            QDate da;
            static_cast<QDateEdit *>( editor_ )->setDate( da.currentDate() );
          }
          break;
          ...
          case QVariant::Time: {
            ...
            static_cast<QTimeEdit *>( editor_ ) ->setTime( timeNow );
          }
          break;
...
QString FLTableDB::tdbFilterBuildWhere() {
  ...
  for ( int i = 0; i < rCount; ++i ) {
    ...
    cond = static_cast<QComboBox *>( tdbFilter->cellWidget( i, 1 ) );
    ...
    switch ( type ) {
      ...
      case QVariant::StringList: {
        ...
        if ( ol ) {
          if ( condType == FLTableDB::FromTo ) {
            QComboBox * editorOp1 = static_cast<QComboBox *>( tdbFilter->cellWidget( i, 3 ) );
            QComboBox * editorOp2 = static_cast<QComboBox *>( tdbFilter->cellWidget( i, 4 ) );
            ...
            QComboBox * editorOp1 = static_cast<QComboBox *>( tdbFilter->cellWidget( i, 2 ) );
            arg2 = cursor_->db()->manager()->formatValue( type, editorOp1->currentText(), true );
          }
        } else {
          if ( condType == FLTableDB::FromTo ) {
            FLLineEdit * editorOp1 = static_cast<FLLineEdit *>( tdbFilter->cellWidget( i, 3 ) );
            FLLineEdit * editorOp2 = static_cast<FLLineEdit *>( tdbFilter->cellWidget( i, 4 ) );
            arg2 = cursor_->db()->manager()->formatValue( type, editorOp1->text(), true );
            arg4 = cursor_->db()->manager()->formatValue( type, editorOp2->text(), true );
          } else {
            FLLineEdit * editorOp1 = static_cast<FLLineEdit *>( tdbFilter->cellWidget( i, 2 ) );
            arg2 = cursor_->db()->manager()->formatValue( type, editorOp1->text(), true );
          }
        }
      }
      ...
      case FLFieldMetaData::Serial:
        if ( condType == FLTableDB::FromTo ) {
          FLSpinBox * editorOp1 = static_cast<FLSpinBox *>( tdbFilter->cellWidget( i, 3 ) );
          FLSpinBox * editorOp2 = static_cast<FLSpinBox *>( tdbFilter->cellWidget( i, 4 ) );
          arg2 = QString::number( editorOp1->value() );
          arg4 = QString::number( editorOp2->value() );
        } else {
          FLSpinBox * editorOp1 = static_cast<FLSpinBox *>( tdbFilter->cellWidget( i, 2 ) );
          arg2 = QString::number( editorOp1->value() );
        }
        break;
      ...
      case QVariant::Date: {
        if ( condType == FLTableDB::FromTo ) {
          QDateEdit * editorOp1 = static_cast<QDateEdit *>( tdbFilter->cellWidget( i, 3 ) );
          QDateEdit * editorOp2 = static_cast<QDateEdit *>( tdbFilter->cellWidget( i, 4 ) );
          ...
          QDateEdit * editorOp1 = static_cast<QDateEdit *>( tdbFilter->cellWidget( i, 2 ) );
          arg2 = cursor_->db()->manager()->formatValue( type, QVariant( editorOp1->date().toString( "dd-MM-yyyy" ) ) );
        }
      }
      break;
      case QVariant::Time: {
        if ( condType == FLTableDB::FromTo ) {
          QTimeEdit * editorOp1 = static_cast<QTimeEdit *>( tdbFilter->cellWidget( i, 3 ) );
          QTimeEdit * editorOp2 = static_cast<QTimeEdit *>( tdbFilter->cellWidget( i, 4 ) );
          ...
          QTimeEdit * editorOp1 = static_cast<QTimeEdit *>( tdbFilter->cellWidget( i, 2 ) );
          arg2 = cursor_->db()->manager()->formatValue( type, QVariant( editorOp1->time().toString( Qt::ISODate ) ) );
        }
      }
      break;
      ...
      case QVariant::Bool: {
        FLCheckBox * editorOp1 = static_cast<FLCheckBox *>( tdbFilter->cellWidget( i, 2 ) );
    ...
    if ( condType == FLTableDB::Contains )
      ...
      if ( condType == FLTableDB::Starts )
        condValue += " like '" + arg2.replace( "'", "" ) + "%%'";
      else
        if ( condType == FLTableDB::End )
          condValue += " like '%%" + arg2.replace( "'", "" ) + "'";
        else
          if ( condType == FLTableDB::Equal )
            condValue += " = " + arg2;
          else
            if ( condType == FLTableDB::Dist )
              condValue += " <> " + arg2;
            else
              if ( condType == FLTableDB::Greater )
                condValue += " > " + arg2;
              else
                if ( condType == FLTableDB::Less )
                  condValue += " < " + arg2;
                else
                  if ( condType == FLTableDB::FromTo )
                    condValue += " >= " + arg2 + " and " + fieldArg + " <= " + arg4;
                  else
                    if ( condType == FLTableDB::Null )
                      condValue += " is null ";
                    else
                      if ( condType == FLTableDB::NotNull )
...
void FLTableDB::initFakeEditor() {
  if ( !fakeEditor_ ) {
    ...
    static_cast<QTextEdit *>( fakeEditor_ )->setTabChangesFocus( true );
  ...
  static_cast<QTextEdit *>( fakeEditor_ )->setText( prty );
}
