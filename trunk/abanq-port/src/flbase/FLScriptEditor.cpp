...
FLScriptEditor::FLScriptEditor( const QString & nFS, QWidget *parent, const char *name )
    ...
    nameObject_( QString::null ), lastSearch( QString::null ) {
  FLApplication * flapp = static_cast<FLApplication *>( qApp );
  nameObject_ = *( flapp->nameObjectScript( nFS ) );
  QString code( flapp->db()->managerModules()->getScript( nFS, 0, false ) );
  ed = new QSEditor( frMain );
  ed->setText( code );
  ed->setInterpreter( flapp->project() ->interpreter(), flapp->project() ->object( nameObject_ ) );
