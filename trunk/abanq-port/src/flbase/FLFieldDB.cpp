...
FLLineEdit::FLLineEdit( QWidget * parent, const char *name ) : QLineEdit( parent, name ) {}
...
FLPixmapView::FLPixmapView( QWidget * parent ) : QScrollView( parent ), autoScaled_( false ), path_( QString::null ) {
...
void FLPixmapView::drawContents( QPainter * p, int cx, int cy, int cw, int ch ) {
  ...
  if ( autoScaled_ ) {
    ...
    if ( !pixmapView_.isNull() && pixmapView_.width() == newWidth && pixmapView_.height() == newHeight )
      return;
    ...
    if ( !pixmapView_.isNull() )
      p->drawPixmap(( width() / 2 ) - ( pixmapView_.width() / 2 ), ( height() / 2 ) - ( pixmapView_.height() / 2 ), pixmapView_ );
  } else {
    if ( !pixmap_.isNull() )
      p->drawPixmap(( width() / 2 ) - ( pixmap_.width() / 2 ), ( height() / 2 ) - ( pixmap_.height() / 2 ), pixmap_ );
  }
}
...
FLFieldDB::FLFieldDB( QWidget * parent, const char *name ) : FLWidgetFieldDB( parent, name ),
    editor_( 0 ), fieldName_( QString::null ), tableName_( QString::null ),
    actionName_( QString::null ), foreignField_( QString::null ),
    fieldRelation_( QString::null ), filter_( QString::null ), cursor_( 0 ),
    cursorAux( 0 ), cursorInit( false ), cursorAuxInit( false ), topWidget_( 0 ),
    showed( false ), showAlias_( true ), datePopup_( 0 ), dateFrame_( 0 ),
    datePickerOn_( false ), autoComPopup_( 0 ), autoComFrame_( 0 ),
    accel_( 0 ), keepDisabled_( false ), editorImg_( 0 ), pbAux_( 0 ), pbAux2_( 0 ), pbAux3_( 0 ),
    fieldAlias_( QString::null ), showEditor_( true ) {
  ...
  if ( !topWidget_ ) {
    ...
    cursor_ = static_cast<FLFormDB *>( topWidget_ )->cursor();
...
void FLFieldDB::updateValue( const QString & t ) {
  ...
  if ( ol && editor_ )
    tAux = field->optionsList()[ static_cast<QComboBox *>( editor_ )->currentItem()];

  if ( !cursor_->bufferIsNull( fieldName_ ) ) {
    if ( tAux == cursor_->valueBuffer( fieldName_ ).toString() ) {
#ifdef FL_TEST
      static_cast<FLApplication *>( qApp )->continueTesting( FLTester::FIELD_READY, this->fieldName() );
#endif
      return ;
    }
  } else
  ...
  if ( field->type() == QVariant::String && !ol ) {
    if ( s.startsWith( " " ) ) {
      disconnect( cursor_, SIGNAL( bufferChanged( const QString & ) ), this, SLOT( refreshQuick( const QString & ) ) );
...
void FLFieldDB::updateValue() {
  ...
  if ( !editor_->isA( "QTextEdit" ) )
    return ;
  QString t = static_cast<QTextEdit *>( editor_ )->text();
  if ( !cursor_->bufferIsNull( fieldName_ ) ) {
...
void FLFieldDB::setTextFormat( const int & f ) {
  ...
  if ( field && editor_ && field->type() == QVariant::StringList )
    static_cast<QTextEdit *>( editor_ )->setTextFormat(( Qt::TextFormat ) f );
}
...
int FLFieldDB::textFormat() const {
  ...
  if ( field && editor_ && field->type() == QVariant::StringList )
    return static_cast<QTextEdit *>( editor_ )->textFormat();

  return PlainText;
}

void FLFieldDB::setValue( QVariant v ) {
  if ( !cursor_ )
    return;
  ...
  if ( field->hasOptionsList() ) {
    ...
    if ( idxItem == -1 )
      idxItem = v.toInt();
    static_cast<QComboBox *>( editor_ )->setCurrentItem( idxItem );
    updateValue( static_cast<QComboBox *>( editor_ )->currentText() );
#ifdef FL_TEST
    static_cast<FLApplication *>( qApp )->continueTesting( FLTester::FIELD_READY, this->name() );
#endif
    return;
  }
  ...
  switch ( type ) {
    ...
    case QVariant::String:
      if ( editor_ ) {
        bool doHome = ( static_cast<FLLineEdit *>( editor_ )->text().isEmpty() );
        static_cast<FLLineEdit *>( editor_ )->setText( isNull ? QString() : v.toString() );
        if ( doHome )
          static_cast<FLLineEdit *>( editor_ )->home( false );
      }
      break;
    case QVariant::StringList:
      if ( !editor_ )
        return ;
      static_cast<QTextEdit *>( editor_ )->setText( isNull ? QString() : v.toString() );
      break;
    case QVariant::Double:
      if ( editor_ ) {
        ...
        static_cast<FLLineEdit *>( editor_ )->setText( s );
      }
      break;
    case FLFieldMetaData::Serial:
      if ( editor_ )
        static_cast<FLSpinBox *>( editor_ )->setValue( isNull ? 0 : v.toUInt() );
      break;
    ...
    case QVariant::Date:
      if ( editor_ )
        static_cast<FLDateEdit *>( editor_ )->setDate( isNull ? QDate() : v.toDate() );
      break;
    case QVariant::Time:
      if ( editor_ )
...
QVariant FLFieldDB::value() {
  ...
  switch ( type ) {
    ...
    case QVariant::StringList:
      if ( editor_ )
        v = static_cast<FLLineEdit *>( editor_ )->text();
      break;
    case FLFieldMetaData::Serial:
      if ( editor_ )
        v = static_cast<FLSpinBox *>( editor_ )->value();
      break;
    case QVariant::Pixmap:
    ...
    case QVariant::Date:
      if ( editor_ )
        v = static_cast<FLDateEdit *>( editor_ )->date().toString( Qt::ISODate );
      break;
    case QVariant::Time:
      if ( editor_ )
...
void FLFieldDB::selectAll() {
  ...
  switch ( field->type() ) {
    ...
    case QVariant::String:
      if ( editor_ )
        static_cast<FLLineEdit *>( editor_ )->selectAll();
      break;
    case FLFieldMetaData::Serial:
      if ( editor_ )
        static_cast<FLSpinBox *>( editor_ )->selectAll();
      break;
  }
}
...
void FLFieldDB::initCursor() {
  if ( !tableName_.isEmpty() && foreignField_.isEmpty() && fieldRelation_.isEmpty() ) {
    ...
    if ( cursor_ )
      ...
      if ( !topWidget_ )
        return;
      cursor_ = new FLSqlCursor( tableName_, true, FLSqlConnections::database()->connectionName(), 0, 0, this );
    }
  ...
  if ( tableName_.isEmpty() || foreignField_.isEmpty()
    ...
    if ( !foreignField_.isEmpty() && !fieldRelation_.isEmpty() ) {
      if ( showed )
        disconnect( cursor_, SIGNAL( bufferChanged( const QString & ) ), this, SLOT( refresh( const QString & ) ) );
  ...
  if ( !cursorAux ) {
    ...
    if ( !rMD ) {
      ...
      FLRelationMetaData * testM1 = cursor_->metadata()->relation( foreignField_, fieldRelation_, tableName_ );
      if ( testM1 )
        checkIntegrity = ( testM1->cardinality() == FLRelationMetaData::RELATION_M1 );
      FLFieldMetaData * fMD = tMD->field( fieldRelation_ );
      if ( fMD ) {
        rMD = new FLRelationMetaData( curName, foreignField_, FLRelationMetaData::RELATION_1M, false, false, checkIntegrity );
  ...
  switch ( type ) {
    ...
    case QVariant::String:
      if ( ol ) {
        ...
        static_cast<QComboBox *>( editor_ )->setAutoCompletion( true );
        static_cast<QComboBox *>( editor_ )->setFont( qApp->font() );
        ...
        static_cast<QComboBox *>( editor_ )->insertStringList( olTranslated );

        editor_->installEventFilter( this );
        if ( showed )
          disconnect( editor_, SIGNAL( activated( const QString & ) ), this, SLOT( updateValue( const QString & ) ) );
        connect( editor_, SIGNAL( activated( const QString & ) ), this, SLOT( updateValue( const QString & ) ) );
      } else {
        editor_ = new FLLineEdit( this, "editor" );

        static_cast<FLLineEdit *>( editor_ )->setFont( qApp->font() );
        static_cast<FLLineEdit *>( editor_ )->type = type;
        editor_->installEventFilter( this );

        if ( type == QVariant::Double ) {
          static_cast<FLLineEdit *>( editor_ )->setValidator( new FLDoubleValidator( 0, pow( 10, partInteger ) - 1, partDecimal, editor_ ) );
          static_cast<FLLineEdit *>( editor_ )->setAlignment( Qt::AlignRight );
        } else {
          if ( type == QVariant::UInt || type == QVariant::Int ) {
            if ( type == QVariant::UInt )
              static_cast<FLLineEdit *>( editor_ )->setValidator( new FLUIntValidator( 0, (( int ) pow( 10, partInteger ) - 1 ), editor_ ) );
            else
              static_cast<FLLineEdit *>( editor_ )->setValidator( new FLIntValidator((( int )( pow( 10, partInteger ) - 1 ) *( -1 ) ),
                  (( int ) pow( 10, partInteger ) - 1 ), editor_ ) );
            static_cast<FLLineEdit *>( editor_ )->setAlignment( Qt::AlignRight );
          } else {
            static_cast<FLLineEdit *>( editor_ )->setMaxLength( len );
            if ( !rX.isEmpty() ) {
              QRegExp r( rX );
              static_cast<FLLineEdit *>( editor_ )->setValidator( new QRegExpValidator( r, editor_ ) );
            }
            static_cast<FLLineEdit *>( editor_ )->setAlignment( Qt::AlignLeft );
        ...
        if ( showed ) {
          ...
          disconnect( editor_, SIGNAL( textChanged( const QString & ) ), this, SLOT( emitTextChanged( const QString & ) ) );
        }
        ...
        connect( editor_, SIGNAL( textChanged( const QString & ) ), this, SLOT( emitTextChanged( const QString & ) ) );
        if ( hasPushButtonDB ) {
          if ( showed )
    ...
    case FLFieldMetaData::Serial:
      ...
      static_cast<FLSpinBox *>( editor_ )->setFont( qApp->font() );
      static_cast<FLSpinBox *>( editor_ )->setMaxValue((( int ) pow( 10, partInteger ) - 1 ) );
    ...
    case QVariant::Pixmap:
      if ( cursor_->modeAccess() != FLSqlCursor::BROWSE ) {
        ...
        if ( !pbAux3_ ) {
          ...
          pbAux3_->setSizePolicy( QSizePolicy( QSizePolicy::Fixed, QSizePolicy::Fixed, pbAux3_->sizePolicy().hasHeightForWidth() ) );
        ...
        if ( !pbAux_ ) {
          ...
          pbAux_->setSizePolicy( QSizePolicy( QSizePolicy::Fixed, QSizePolicy::Fixed, pbAux_->sizePolicy().hasHeightForWidth() ) );
        ...
        if ( !pbAux2_ ) {
          ...
          pbAux2_->setSizePolicy( QSizePolicy( QSizePolicy::Fixed, QSizePolicy::Fixed, pbAux2_->sizePolicy().hasHeightForWidth() ) );
    ...
    case QVariant::Date:
      ...
      static_cast<FLDateEdit *>( editor_ )->setFont( qApp->font() );

      editor_-> setSizePolicy( QSizePolicy(( QSizePolicy::SizeType ) 7, ( QSizePolicy::SizeType ) 0,
                                           editor_->sizePolicy().hasHeightForWidth() ) );
      FLWidgetFieldDBLayout->insertWidget( 1, editor_ );

      static_cast<FLDateEdit *>( editor_ )->setOrder( QDateEdit::DMY );
      static_cast<FLDateEdit *>( editor_ )->setAutoAdvance( true );
      static_cast<FLDateEdit *>( editor_ )->setSeparator( "-" );
      editor_->installEventFilter( this );
      ...
      if ( cursor_->modeAccess() != FLSqlCursor::BROWSE ) {
        if ( !pbAux_ ) {
          pbAux_ = new QPushButton( this, "pbAux" );
          pbAux_->setFlat( true );
          pbAux_->setSizePolicy( QSizePolicy(( QSizePolicy::SizeType )0, ( QSizePolicy::SizeType )0, 0, 0,
      ...
      if ( showed )
        disconnect( editor_, SIGNAL( valueChanged( const QDate & ) ), this, SLOT( updateValue( const QDate & ) ) );
      ...
      if ( cursor_->modeAccess() == FLSqlCursor::INSERT && !field->allowNull() ) {
        QVariant defVal( field->defaultValue() );
        if ( !defVal.isValid() || defVal.isNull() )
          static_cast<FLDateEdit *>( editor_ )->setDate( QDate::currentDate() );
        else
          static_cast<FLDateEdit *>( editor_ )->setDate( defVal.toDate() );
      }
      break;
    ...
    case QVariant::Time: {
      ...
      static_cast<QTimeEdit *>( editor_ )->setFont( qApp->font() );
      static_cast<QTimeEdit *>( editor_ )->setAutoAdvance( true );
      ...
      if ( showed )
        disconnect( editor_, SIGNAL( valueChanged( const QTime & ) ), this, SLOT( updateValue( const QTime & ) ) );
      ...
      if ( cursor_->modeAccess() == FLSqlCursor::INSERT && !field->allowNull() ) {
        ...
        if ( !defVal.isValid() || defVal.isNull() )
          ...
          static_cast<QTimeEdit *>( editor_ )->setTime( defVal.toTime() );
      }
    }
    break;
    ...
    case QVariant::StringList:
      ...
      static_cast<QTextEdit *>( editor_ )->setFont( qApp->font() );
      static_cast<QTextEdit *>( editor_ )->setTabChangesFocus( true );
    ...
    case QVariant::Bool:
      ...
      static_cast<QCheckBox *>( editor_ )->setFont( qApp->font() );
      editor_->installEventFilter( this );

      editor_->setMinimumWidth( fontMetrics().width( tMD->fieldNameToAlias( fieldName_ ) ) + fontMetrics().maxWidth() * 2 );
      editor_->setSizePolicy( QSizePolicy(( QSizePolicy::SizeType ) 7, ( QSizePolicy::SizeType ) 0,
                                          editor_->sizePolicy().hasHeightForWidth() ) );
      FLWidgetFieldDBLayout->addWidget( editor_ );
...
void FLFieldDB::searchValue() {
  ...
  if ( fMD ) {
    ...
    if ( v.toString().isEmpty() ) {
      QMessageBox::warning( qApp->focusWidget(), tr( "Aviso" ),
                            tr( "Debe indicar un valor para %1" ).arg( fMD->alias() ), QMessageBox::Ok, 0, 0 );
      return ;
    }

    c = new FLSqlCursor( fMD->relationM1()->foreignTable(), true, cursor_->db()->connectionName() );
    c->select( cursor_->db()->manager()->formatAssignValue( fMD->relationM1()->foreignField(), fMD, v, true ) );
    if ( c->size() > 0 )
      c->next();
  ...
  delete lObjs;
  if ( fMD && obj ) {
    static_cast<FLTableDB *>( obj )->setTableName( field->relationM1()->foreignTable() );
    static_cast<FLTableDB *>( obj )->setFieldRelation( field->associatedFieldFilterTo() );
    static_cast<FLTableDB *>( obj )->setForeignField( fMD->relationM1()->foreignField() );
    if ( fMD->relationM1()->foreignTable() == tMD->name() )
      static_cast<FLTableDB *>( obj )->setReadOnly( true );
  }
  ...
  if ( f->mainWidget() ) {
    ...
    if ( field->type() == QVariant::String && !curValue.toString().isEmpty() && obj ) {
      static_cast<FLTableDB *>( obj )->setInitSearch( curValue.toString() );
      static_cast<FLTableDB *>( obj )->putFirstCol( field->relationM1()->foreignField() );
    }
...
void FLFieldDB::refreshQuick( const QString & fN ) {
  ...
  switch ( type ) {
    case QVariant::Double:
      if ( v.toDouble() == static_cast<FLLineEdit *>( editor_ )->text().toDouble() )
        return ;
      disconnect( editor_, SIGNAL( textChanged( const QString & ) ), this, SLOT( updateValue( const QString & ) ) );
      {
        QString s;
    ...
    case QVariant::String: {
      ...
      if ( ol ) {
        ...
        if ( v.toString() == static_cast<FLLineEdit *>( editor_ )->text() )
          return ;
        doHome = ( static_cast<FLLineEdit *>( editor_ )->text().isEmpty() );
      }
      disconnect( editor_, SIGNAL( textChanged( const QString & ) ), this, SLOT( updateValue( const QString & ) ) );
      if ( v.isValid() && !v.isNull() ) {
        if ( ol )
          static_cast<QComboBox *>( editor_ )->setCurrentItem( field->optionsList().findIndex( v.toString() ) );
        else
          static_cast<FLLineEdit *>( editor_ )->setText( v.toString() );
      } else {
        if ( ol )
          static_cast<QComboBox *>( editor_ )->setCurrentItem( 0 );
        else
          static_cast<FLLineEdit *>( editor_ )->setText( QString::null );
      }
      if ( !ol && doHome )
        static_cast<FLLineEdit *>( editor_ )->home( false );
      connect( editor_, SIGNAL( textChanged( const QString & ) ), this, SLOT( updateValue( const QString & ) ) );
    ...
    case QVariant::UInt:
      if ( v.toUInt() == static_cast<FLLineEdit *>( editor_ )->text().toUInt() )
        return ;
      disconnect( editor_, SIGNAL( textChanged( const QString & ) ), this, SLOT( updateValue( const QString & ) ) );
      {
        QString s;
    ...
    case QVariant::Int:
      if ( v.toInt() == static_cast<FLLineEdit *>( editor_ )->text().toInt() )
        return ;
      disconnect( editor_, SIGNAL( textChanged( const QString & ) ), this, SLOT( updateValue( const QString & ) ) );
      {
        QString s;
    ...
    case FLFieldMetaData::Serial:
      if ( v.toInt() == static_cast<FLSpinBox *>( editor_ )->value() )
        return ;
      disconnect( editor_, SIGNAL( valueChanged( const QString & ) ), this, SLOT( updateValue( const QString & ) ) );
      static_cast<FLSpinBox *>( editor_ )->setValue( v.toInt() );
      connect( editor_, SIGNAL( valueChanged( const QString & ) ), this, SLOT( updateValue( const QString & ) ) );
      break;
    ...
    case QVariant::Date:
      if ( v.toDate() == static_cast<FLDateEdit *>( editor_ )->date() )
        return ;
      disconnect( editor_, SIGNAL( valueChanged( const QDate & ) ), this, SLOT( updateValue( const QDate & ) ) );
      static_cast<FLDateEdit *>( editor_ )->setDate( v.toDate() );
      connect( editor_, SIGNAL( valueChanged( const QDate & ) ), this, SLOT( updateValue( const QDate & ) ) );
      break;
    ...
    case QVariant::StringList:
      if ( v.toString() == static_cast<QTextEdit *>( editor_ )->text() )
        return ;
      disconnect( editor_, SIGNAL( textChanged() ), this, SLOT( updateValue() ) );
      static_cast<QTextEdit *>( editor_ )->setText( v.toString() );
      connect( editor_, SIGNAL( textChanged() ), this, SLOT( updateValue() ) );
      break;
...
void FLFieldDB::refresh( const QString & fN ) {
  ...
  if ( fN.isEmpty() ) {
    ...
    if ( !cursorAux && fN.lower() == fieldRelation_.lower() ) {
      ...
      FLTableMetaData *tmd = cursor_->db()->manager()->metadata( field->relationM1()->foreignTable() );
      if ( !tmd )
        return ;
  ...
  switch ( type ) {
    ...
    case FLFieldMetaData::Serial:
      disconnect( editor_, SIGNAL( valueChanged( const QString & ) ), this, SLOT( updateValue( const QString & ) ) );
      static_cast<FLSpinBox *>( editor_ )->setValue( 0 );
      connect( editor_, SIGNAL( valueChanged( const QString & ) ), this, SLOT( updateValue( const QString & ) ) );
      break;
...
FLDoubleValidator::FLDoubleValidator( QObject * parent, const char
                                      *name ) : QDoubleValidator( parent, name ) {}

FLDoubleValidator::FLDoubleValidator( double bottom, double top, int decimals,
                                      QObject * parent, const char
                                      *name ) : QDoubleValidator( bottom, top,
                                                                      decimals, parent,
                                                                      name ) {}
...
FLIntValidator::FLIntValidator( int minimum, int maximum, QObject * parent, const char *name ) : QIntValidator( minimum, maximum,
        parent,
        name ) {}
...
FLUIntValidator::FLUIntValidator( int minimum, int maximum, QObject * parent, const char *name ) : QIntValidator( minimum, maximum,
        parent,
        name ) {}
...
void FLFieldDB::setMapValue( const QString & v ) {
  ...
  QString fSN = static_cast<FLFieldDB *>( s )->fieldName();
  FLFieldMetaData *field = tMD->field( fieldName_ );
  ...
  if ( field->relationM1() ) {
    if ( field->relationM1()->foreignTable() != tMD->name() ) {
      rt = field->relationM1()->foreignTable();
      fF = static_cast<FLFieldDB *>( s )->foreignField();
      FLSqlQuery q( 0, cursor_->db()->connectionName() );
      q.setForwardOnly( true );
      q.setTablesList( rt );
...
void FLFieldDB::toggleDatePicker() {
  if ( !dateFrame_ ) {
    ...
    if ( !datePopup_ ) {
      ...
      connect( datePopup_, SIGNAL( dateSelected( const QDate & ) ), static_cast<FLDateEdit *>( editor_ ), SLOT( setDate( const QDate & ) ) );
    }
  }
  ...
  if ( !datePickerOn_ ) {
    ...
    QDate date = static_cast<FLDateEdit *>( editor_ )->date();
    if ( date.isValid() ) {
...
void FLFieldDB::savePixmap( int f ) {
  if ( editorImg_ ) {
    ...
    QString savefilename = QFileDialog::getSaveFileName( filename.lower(), "*." + ext, this, filename, tr( "Guardar imagen como" ) );
    ...
    if ( !autoComPopup_ ) {
      ...
      if ( field ) {
        ...
        if ( !field->relationM1() ) {
          ...
          cur = new FLSqlCursor( tMD->name(), false, cursor_->db()->connectionName(), 0, 0, autoComFrame_ );
        } else {
          autoComFieldName_ = field->relationM1()->foreignField();
          cur = new FLSqlCursor( field->relationM1()->foreignTable(), false, cursor_->db()->connectionName(), 0, 0, autoComFrame_ );
          tMD = cur->metadata();
          field = tMD ? tMD->field( autoComFieldName_ ) : field;
        }

        cur->append( QSqlFieldInfo( autoComFieldName_, FLFieldMetaData::flDecodeType( field->type() ), -1, field->length(), -1 ) );
        ...
        for ( QStringList::Iterator it = fieldsNames.begin(); it != fieldsNames.end(); ++it ) {
          if ( !cur->QSqlCursor::field(( *it ) ) ) {
            field = tMD->field( *it );
            if ( field )
              cur->append( QSqlFieldInfo( field->name(), FLFieldMetaData::flDecodeType( field->type() ), -1,
...
void FLFieldDB::initFakeEditor() {
  bool hasPushButtonDB = ( tableName_.isEmpty() && foreignField_.isEmpty() && fieldRelation_.isEmpty() );
  ...
  if ( !actionName_.isEmpty() )
    prty += "aN:" + actionName_ + ",";
  static_cast<QLineEdit *>( editor_ )->setText( prty );
  static_cast<QLineEdit *>( editor_ )->home( false );
