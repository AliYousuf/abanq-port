...
#ifdef FL_TEST
      ::qt_cast<FLApplication *>( qApp )->continueTesting( FLTester::FIELD_READY, this->fieldName() );
#endif
      return ;
    }
  } else
...
#ifdef FL_TEST
    ::qt_cast<FLApplication *>( qApp )->continueTesting( FLTester::FIELD_READY, this->name() );
#endif
    return;
  }
...
void FLFieldDB::refreshQuick( const QString & fN ) {
  ...
  switch ( type ) {
    ...
    case QVariant::String: {
      ...
      if ( v.isValid() && !v.isNull() ) {
        if ( ol )
          ::qt_cast<QComboBox *>( editor_ )->setCurrentItem( field->optionsList().findIndex( v.toString() ) );
        else
...
void FLFieldDB::setMapValue( const QString & v ) {
  ...
  QString fSN = ::qt_cast<FLFieldDB *>( s )->fieldName();
  FLFieldMetaData *field = tMD->field( fieldName_ );
  ...
  if ( field->relationM1() ) {
    if ( field->relationM1()->foreignTable() != tMD->name() ) {
      rt = field->relationM1()->foreignTable();
      fF = ::qt_cast<FLFieldDB *>( s )->foreignField();
      FLSqlQuery q( 0, cursor_->db()->connectionName() );
      q.setForwardOnly( true );
      q.setTablesList( rt );
...
void FLFieldDB::toggleDatePicker() {
  if ( !dateFrame_ ) {
    ...
    if ( !datePopup_ ) {
      ...
      connect( datePopup_, SIGNAL( dateSelected( const QDate & ) ), ::qt_cast<FLDateEdit *>( editor_ ), SLOT( setDate( const QDate & ) ) );
    }
  }
