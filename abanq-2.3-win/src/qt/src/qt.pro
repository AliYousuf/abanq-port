...
#win32:CONFIG += qt-kernel-lib 

qt-kernel-lib: {
	itarget.target = ../lib/libqt-mt.a
	IMPFLAGS = -Wl,-enable-stdcall-fixup -Wl,-enable-auto-import -Wl,-enable-runtime-pseudo-reloc -fno-exceptions \
			-Wl,-s -Wl,-subsystem,windows -shared -Wl,--export-all-symbols \ 
			-Wl,--out-implib,../lib/libqt-mt.a
	itarget.commands = $(LINK) $$IMPFLAGS $(OBJECTS) $(OBJMOC) -o $(TARGET) 2> /dev/null
	QMAKE_EXTRA_WIN_TARGETS += itarget

	TARGETDEPS += ../lib/qt_kernel3.dll
	ktarget.target = ../lib/qt_kernel3.dll
	ktarget.depends = ../lib/libqt-mt.a
	ktarget.commands = cd kernel && $(MAKE)
	LIBS += -lqt_kernel
	QMAKE_EXTRA_WIN_TARGETS += ktarget
}
else {
