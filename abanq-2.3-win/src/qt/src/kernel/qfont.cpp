...
Q_EXPORT int qt_defaultDpiX()
{
    ...
    dpi = GetDeviceCaps(qt_display_dc(),LOGPIXELSX);
...
#elif defined(Q_WS_WIN)
    dpi = GetDeviceCaps(qt_display_dc(),LOGPIXELSY);
#elif defined(Q_WS_MAC)
...
// #endif
    ...
    int dpi = ( painter->device() ? QPaintDeviceMetrics(painter->device()).logicalDpiY() : qt_defaultDpi() );
