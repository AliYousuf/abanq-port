...
public:
  ...
  QString path() const {
    return QFileInfo( *file ).dirPath( true );
  }
  ...
  QString extension() const {
    return QFileInfo( *file ).extension();
  }
  QString symLink() const {
    QFileInfo f( *file );
    return f.isSymLink() ? f.readLink() : QString();
  }
...
public slots:
  QString cleanDirPath( const QString &path ) const {
    return QDir::cleanDirPath( path );
  }
  QString convertSeparators( const QString &path ) const {
...
public:
  ...
  QString workingDirectory() const {
    return process->workingDirectory().absPath();
  }
...
public slots:
  ...
  bool canReadLineStdout() {
    return process->canReadLineStdout();
  }
  bool canReadLineStderr() {
    return process->canReadLineStderr();
  }

  QString readLineStdout() {
    return process->readLineStdout();
  }
  QString readLineStderr() {
    return process->readLineStderr();
  }
...
//### AbanQ Hack
    QByteArray content8Bit( content.local8Bit() );
    //uint len = ( content8Bit.size() > 0 ?  content8Bit.size() - 1 : 0 );
    uint len = strlen( content8Bit.data() );
    if ( !file.writeBlock( content8Bit.data(), len ) == len )
...
void QSFile::write( const QString &data, int length ) {
  if ( length < 0 )
    length = data.length();
//### AbanQ Hack
  QByteArray data8Bit( data.right( length ).local8Bit() );
  //uint len = ( data8Bit.size() > 0 ?  data8Bit.size() - 1 : 0 );
  uint len = strlen( data8Bit.data() );
  //int written = file->writeBlock( data8Bit, len );
  int written = file->writeBlock( data8Bit.data(), len );
  if ( written != len ) {
