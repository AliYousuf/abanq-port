...
class oficial extends interna {
	...
	function porIVAE(nodo:FLDomNode, campo:String):String {
		return this.ctx.oficial_porIVAE(nodo, campo);
	}
	function seriales(nodo:FLDomNode, campo:String):String {
		return this.ctx.oficial_seriales(nodo, campo);
	}
...
*/
	...
	function borrarAsientoArqueo(curArqueo:FLSqlCursor, valoresDefecto:Array):Boolean {
		return this.ctx.oficial_borrarAsientoArqueo(curArqueo, valoresDefecto);
	}
...
class ifaceCtx extends head {
	...
	function pub_borrarAsientoArqueo(curArqueo:FLSqlCursor, valoresDefecto:Array):Boolean {
		return this.borrarAsientoArqueo(curArqueo, valoresDefecto);
	}
}
...
/*
	if (!curComanda.isNull("idfactura"))
		integracionFac = "Sí";

	if (pendiente != 0 && integracionFac == "No") {
		var res:Number = MessageBox.information(util.translate("scripts", "Va a guardar una venta a cuenta (que no está completamente pagada).\n¿Desea generar la factura y recibos asociados a esta venta a cuenta?"),  MessageBox.Yes, MessageBox.No, MessageBox.Cancel);
...
*/
	if (pendiente != 0 || integracionFac || !curComanda.isNull("idfactura")) {
		return this.iface.sincronizarConFacturacion(curComanda);
	}
...
function oficial_porIVAE(nodo:FLDomNode, campo:String):String
{
	...
	idDocumento = nodo.attributeValue(campo);
	ValIva = util.sqlSelect("tpv_lineascomanda", "distinct iva", "referencia = '" +idDocumento + "'" );
...
function oficial_seriales(nodo:FLDomNode, campo:String):String
{
   var util:FLUtil = new FLUtil;
	...
	tabla = "serialescomanda";
	campoClave = "idtpv_comanda";
	
	idDocumento = nodo.attributeValue(campo);
	var qrySeriales:FLSqlQuery = new FLSqlQuery();
	
	with (qrySeriales) {
		setTablesList(tabla);
		setSelect("referencia, anio, serial_motor, serial_chasis, placa, color");
		setFrom(tabla);
		setWhere(campoClave + " = " + idDocumento + " ORDER BY referencia ");
	}
	
	if (!qrySeriales.exec())
		return false;
		
	var listaSeriales:String = "";
	
	while (qrySeriales.next()) {
	
		if (listaSeriales != "")
			listaSeriales += "\n";
			
       listaSeriales += "Placa: " + qrySeriales.value("placa") + " Serial Motor: " + qrySeriales.value("serial_motor") + "\n" + " Serial Chasis: " + qrySeriales.value("serial_chasis") + " Año: " + qrySeriales.value("anio") + "\n" + " Color: " + qrySeriales.value("color");
	}
...
function oficial_generarAsientoArqueo(curArqueo:FLSqlCursor):Boolean
{
	...
	if (datosAsiento.error == true)
		return false;
/*
	if (!this.iface.generarPartidasCliente(curArqueo, datosAsiento.idasiento, valoresDefecto))
		return false;

	if (!this.iface.generarPartidasVentasIva(curArqueo, datosAsiento.idasiento, valoresDefecto))
		return false;

	if (!this.iface.generarPartidasPago(curArqueo, datosAsiento.idasiento, valoresDefecto))
		return false;
*/
	...
	if (!util.sqlSelect("co_partidas", "idpartida", "idasiento = " + datosAsiento.idasiento)) {
		if (!this.iface.pub_borrarAsientoArqueo(curArqueo))
			return false;
	}
/*
	else {
		MessageBox.information(util.translate("scripts", "Se generó el asiento contable asociado al arqueo.\nPuede consultarlo en la pestaña Contabilidad"), MessageBox.Ok, MessageBox.NoButton);
	}
*/
	return true;
}
...
\end */
function oficial_borrarAsientoArqueo(curArqueo:FLSqlCursor, valoresDefecto:Array):Boolean
{
	var util:FLUtil = new FLUtil;
	var asiento:Array = [];
	var idAsiento:Number = curArqueo.valueBuffer("idasiento");
	if (curArqueo.isNull("idasiento"))
		return true;
