...
function interna_init()
{
	...
	if (cursor.modeAccess() == cursor.Insert) {
		...
		qryarqueos.setSelect("MAX(diahasta), totalcaja, totalmov, sacadodecaja");
		qryarqueos.setFrom("tpv_arqueos");
		qryarqueos.setWhere("idtpv_arqueo <> '" + cursor.valueBuffer("idtpv_arqueo") + "' GROUP BY totalcaja, totalmov, sacadodecaja");
		if (!qryarqueos.exec())
			return;
		if (qryarqueos.first())
			inicio = qryarqueos.value(1) + qryarqueos.value(2) - qryarqueos.value(3);
		cursor.setValueBuffer("inicio", inicio);
...
function interna_validateForm():Boolean
{
	...
	if (cursor.valueBuffer("sacadodecaja") > 0 && cursor.valueBuffer("abierta")) {
		var res:Number = MessageBox.warning(util.translate("scripts", "¿Desea cerrar el arqueo?"),MessageBox.Yes, MessageBox.No, MessageBox.NoButton);
